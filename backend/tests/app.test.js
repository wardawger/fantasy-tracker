...
describe('League dues logic', () => {
  ...
  test('total_paid only sums main league payments', async () => {
    ...
    await db.run('INSERT INTO payments (id, member_id, amount, date, method, league) VALUES (?, ?, ?, ?, ?, ?)',
      [crypto.randomUUID(), memberId, 250, '2026-01-01', 'Venmo', 'main']);
    await db.run('INSERT INTO payments (id, member_id, amount, date, method, league) VALUES (?, ?, ?, ?, ?, ?)',
      [crypto.randomUUID(), memberId, 50, '2026-01-02', 'Venmo', 'survivor']);

    const mainSum = await db.exec("SELECT COALESCE(SUM(amount), 0) FROM payments WHERE member_id = ? AND league = 'main'", [memberId]);
    expect(mainSum[0].values[0][0]).toBe(250);

    const survivorSum = await db.exec("SELECT COALESCE(SUM(amount), 0) FROM payments WHERE member_id = ? AND league = 'survivor'", [memberId]);
  ...
  test('opt-in flags update independently per league', async () => {
    ...
    await db.run('UPDATE members SET survivor_opted_in = 1 WHERE id = ?', [memberId]);

    const result = await db.exec('SELECT survivor_opted_in, chopped_opted_in FROM members WHERE id = ?', [memberId]);
  ...
  test('opt-in existence check finds no row for an unknown member id', async () => {
    ...
    const existing = await db.exec('SELECT id FROM members WHERE id = ?', [memberId]);
    expect(existing[0].values.length).toBe(1);

    const missing = await db.exec('SELECT id FROM members WHERE id = ?', [unknownId]);
  ...
  test('payment creation existence check rejects an unknown member id', async () => {
    ...
    const paymentsBefore = await db.exec('SELECT COUNT(*) FROM payments');
  ...
  test('invalid league value is rejected and does not create a payment or touch total_paid', async () => {
    const db = await getDb(TEST_DB);
    const memberId = crypto.randomUUID();
    await db.run('INSERT INTO members (id, name, sleeper_user_id, total_paid) VALUES (?, ?, ?, ?)',
    ...
    const paymentsAfter = await db.exec('SELECT COUNT(*) FROM payments');
    expect(paymentsAfter[0]?.values[0]?.[0] || 0).toBe(0);

    const memberAfter = await db.exec('SELECT total_paid FROM members WHERE id = ?', [memberId]);
  ...
  test('editing a payment recalculates total_paid scoped to main league', async () => {
    ...
    await db.run('UPDATE members SET total_paid = 100 WHERE id = ?', [memberId]);
    ...
    const sumResult = await db.exec("SELECT COALESCE(SUM(amount), 0) FROM payments WHERE member_id = ? AND league = 'main'", [memberId]);
    ...
    const updatedPayment = await db.exec('SELECT amount, date, method, notes, league FROM payments WHERE id = ?', [paymentId]);
  ...
  test('editing an unknown payment id would be rejected before any UPDATE', async () => {
    ...
    const existing = await db.exec('SELECT member_id, league FROM payments WHERE id = ?', [unknownId]);
  ...
  test('chopped champion payout is opted-in member count times the chopped due', async () => {
    const db = await getDb(TEST_DB);
    await db.run('INSERT INTO members (id, name, sleeper_user_id, chopped_opted_in) VALUES (?, ?, ?, ?)',
  ...
  test('survivor champion is resolved only when exactly one opted-in member remains alive', async () => {
    const db = await getDb(TEST_DB);
    await db.run('INSERT INTO members (id, name, sleeper_user_id, survivor_opted_in) VALUES (?, ?, ?, ?)',
      [crypto.randomUUID(), 'Member A', 'sleeper_a', 1]);
    await db.run('INSERT INTO members (id, name, sleeper_user_id, survivor_opted_in) VALUES (?, ?, ?, ?)',
      [crypto.randomUUID(), 'Member B', 'sleeper_b', 1]);
